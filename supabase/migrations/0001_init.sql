...
drop policy if exists "images: owner all" on public.images;
create policy "images: owner all" on public.images
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "image_tags: owner all" on public.image_tags;
...
drop policy if exists "images bucket: owner select" on storage.objects;
...
drop policy if exists "images bucket: owner insert" on storage.objects;
...
drop policy if exists "images bucket: owner update" on storage.objects;
...
drop policy if exists "images bucket: owner delete" on storage.objects;
